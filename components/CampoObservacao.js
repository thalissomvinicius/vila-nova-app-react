...
export default function CampoObservacao({ field, value, onChange, error, visualMode }) {
  ...
  const addPhoto = async (useCamera) => {
    ...
    try {
      const photo = await captureImageWithGps(useCamera);
      if (!photo) return;
      ...
      onChange({
        ...
        fotos: [
          ...current.fotos,
          {
            ...photo,
            campo_id: field.id,
            source: useCamera ? 'observacao_camera' : 'observacao_galeria',
          },
        ],
      });
    } catch (captureError) {
      if (captureError?.message === 'permissao_media') {
        Alert.alert('Permissão necessária', 'O aplicativo precisa de acesso à câmera e galeria.');
      } else {
