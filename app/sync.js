...
export default function Sync() {
  ...
  const pendingItems = coletaQueue.filter((item) => item.status === 'pendente');
  const failedItems = coletaQueue.filter((item) => item.status === 'erro');
  const sentItems = coletaQueue.filter((item) => item.status === 'sincronizado' || item.status === 'enviando');
...
function getVisualStatus(status) {
  ...
  if (status === 'enviando') return 'ready';
  return 'waiting';
}
